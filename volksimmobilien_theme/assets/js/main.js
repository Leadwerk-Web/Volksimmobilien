...
(function () {
  ...
  // #region agent log
  if (document.body.classList.contains('page-kauf')) {
    var kaufCtaRows = [
      { id: 'hero', selector: '#hero .hero-actions a' },
      { id: 'einstieg', selector: '#einstieg .section-cta-row a' },
      { id: 'kaeufergruppen', selector: '#kaeufergruppen .section-cta-row a' },
      { id: 'region', selector: '#region .section-cta-row a' }
    ];
    var kaufCtaState = kaufCtaRows.map(function (row) {
      return {
        section: row.id,
        buttons: Array.prototype.map.call(document.querySelectorAll(row.selector), function (el) {
          return {
    ...
    var angeboteSection = document.getElementById('angebote');
    fetch('http://127.0.0.1:7890/ingest/7bd5d55a-1af6-43e4-b125-c9dae8c878b4', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Debug-Session-Id': '4d0610' },
      body: JSON.stringify({
        sessionId: '4d0610',
        runId: 'kauf-cta-verify',
        hypothesisId: 'H1',
        location: 'js/main.js:page-kauf',
        message: 'Kaufen CTA visibility snapshot',
        data: {
          angebotSectionHidden: !angeboteSection || window.getComputedStyle(angeboteSection).display === 'none',
          rows: kaufCtaState
        },
        timestamp: Date.now()
      })
  ...
  if (document.body.classList.contains('page-verkauf')) {
    var verkaufRows = [
      { id: 'einstieg', selector: '#einstieg .section-cta-row a' },
      { id: 'objektarten', selector: '#objektarten .section-cta-row a' },
      { id: 'region', selector: '#region .section-cta-row a' },
      { id: 'diskret', selector: '#diskret .section-cta-row a' }
    ];
    var verkaufState = verkaufRows.map(function (row) {
      return {
        section: row.id,
        buttons: Array.prototype.map.call(document.querySelectorAll(row.selector), function (el) {
          return {
    ...
    var immoPanels = Array.prototype.map.call(
      document.querySelectorAll('#objektarten .immobilien-accordion__panel'),
      function (panel) {
        return {
          hasImageVar: (panel.getAttribute('style') || '').indexOf('immo-panel-image') !== -1,
          mediaHidden: panel.querySelector('.immobilien-accordion__media')
            ? window.getComputedStyle(panel.querySelector('.immobilien-accordion__media')).display === 'none'
            : null
        };
      }
    );
    fetch('http://127.0.0.1:7890/ingest/7bd5d55a-1af6-43e4-b125-c9dae8c878b4', {
      ...
      body: JSON.stringify({
        sessionId: '4d0610',
        runId: 'verkauf-cta-verify',
        hypothesisId: 'V1',
        location: 'js/main.js:page-verkauf',
        message: 'Verkaufen CTA and accordion snapshot',
        data: { rows: verkaufState, immoPanels: immoPanels },
        timestamp: Date.now()
      })
    }).catch(function () {});
  }
