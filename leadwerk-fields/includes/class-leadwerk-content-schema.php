...
class Leadwerk_Content_Schema {
	...
	public static function get_groups() {
		...
		$groups = array(
			...
			/* ───────────────────────────────────────────────
			 * volksimmobilien — statische Root-HTML-Seiten
			 * ─────────────────────────────────────────────── */

			'volks_home_sections'         => array(
				'label'       => 'volksimmobilien Startseite',
				'description' => 'Startseite mit einzelnen Text- und Bildfeldern pro Sektion.',
				'source_keys' => array( 'volks-home-v1' ),
				'import_mode' => 'volks_multi_section',
				'layouts'     => class_exists( 'Leadwerk_Volks_Section_Schema' )
					? Leadwerk_Volks_Section_Schema::home_layouts()
					: array( 'html_section' => self::layout_volks_html_section() ),
			),
			'volks_bewerten_sections'     => array(
				'label'       => 'volksimmobilien Bewerten',
				'description' => 'Sektionen der Bewertungsseite.',
				'source_keys' => array( 'volks-bewerten-v1' ),
				'import_mode' => 'volks_multi_section',
				'layouts'     => class_exists( 'Leadwerk_Volks_Section_Schema' )
					? Leadwerk_Volks_Section_Schema::landing_layouts()
					: array( 'html_section' => self::layout_volks_html_section() ),
			),
			'volks_kaufen_sections'       => array(
				'label'       => 'volksimmobilien Kaufen',
				'description' => 'Sektionen der Kauf-Landingpage.',
				'source_keys' => array( 'volks-kaufen-v1' ),
				'import_mode' => 'volks_multi_section',
				'layouts'     => class_exists( 'Leadwerk_Volks_Section_Schema' )
					? Leadwerk_Volks_Section_Schema::landing_layouts()
					: array( 'html_section' => self::layout_volks_html_section() ),
			),
			'volks_verkaufen_sections'    => array(
				'label'       => 'volksimmobilien Verkaufen',
				'description' => 'Sektionen der Verkauf-Landingpage.',
				'source_keys' => array( 'volks-verkaufen-v1' ),
				'import_mode' => 'volks_multi_section',
				'layouts'     => class_exists( 'Leadwerk_Volks_Section_Schema' )
					? Leadwerk_Volks_Section_Schema::landing_layouts()
					: array( 'html_section' => self::layout_volks_html_section() ),
			),
			'volks_mallorca_sections'     => array(
				'label'       => 'volksimmobilien Mallorca',
				'description' => 'Sektionen der Mallorca-Landingpage.',
				'source_keys' => array( 'volks-mallorca-v1' ),
				'import_mode' => 'volks_multi_section',
				'layouts'     => class_exists( 'Leadwerk_Volks_Section_Schema' )
					? Leadwerk_Volks_Section_Schema::landing_layouts()
					: array( 'html_section' => self::layout_volks_html_section() ),
			),
			'volks_ausland_sections'      => array(
				'label'       => 'volksimmobilien Ausland',
				'description' => 'Sektionen der Ausland-Landingpage (Mallorca & Kroatien).',
				'source_keys' => array( 'volks-ausland-v1' ),
				'import_mode' => 'volks_multi_section',
				'layouts'     => class_exists( 'Leadwerk_Volks_Section_Schema' )
					? Leadwerk_Volks_Section_Schema::landing_layouts()
					: array( 'html_section' => self::layout_volks_html_section() ),
			...
			'volks_impressum_page'        => array(
				...
				'source_keys'       => array( 'volks-impressum-v1' ),
				'sync_post_content' => true,
				'fields'            => array(
					'headline' => self::text( 'Seitenueberschrift' ),
					'content'  => self::editor( 'Inhalt' ),
			...
			'volks_datenschutz_page'      => array(
				'label'             => 'volksimmobilien Datenschutz',
				'description'       => 'Datenschutzerklaerung bearbeiten.',
				'source_keys'       => array( 'volks-datenschutz-v1' ),
				'sync_post_content' => true,
				'fields'            => array(
					'headline' => self::text( 'Seitenueberschrift' ),
					'content'  => self::editor( 'Inhalt' ),
			...
			'volks_danke_page'            => array(
				'label'             => 'volksimmobilien Danke',
				'description'       => 'Bestaetigungsseite nach Formular (WPForms Redirect).',
				'source_keys'       => array( 'volks-danke-v1' ),
				'sync_post_content' => true,
				'fields'            => array(
					'headline' => self::text( 'Seitenueberschrift' ),
					'content'  => self::editor( 'Inhalt' ),
			...
			'volks_404_page'              => array(
				'label'             => 'volksimmobilien 404',
				'description'       => 'Inhalt fuer die Fehlerseite (WordPress 404 Template).',
				'source_keys'       => array( 'volks-404-v1' ),
				'sync_post_content' => true,
				'fields'            => array(
					'headline' => self::text( 'Seitenueberschrift' ),
					'content'  => self::editor( 'Inhalt' ),
	...
	/**
	 * Build post_content HTML for volks legal / utility pages (admin fallback).
	 *
	 * @param array<string,mixed> $value headline + content field values.
	 * @return string
	 */
	public static function build_volks_legal_sync_post_content( $value ) {
		if ( ! is_array( $value ) ) {
			return '';
		}
		...
		$html     = '<section class="section legal-content-section"><div class="container"><article class="legal-prose">';

		if ( '' !== $headline ) {
			$html .= '<h1 class="legal-prose__title">' . esc_html( $headline ) . '</h1>';
		}

		$html .= '<div class="legal-prose__body">' . $content . '</div></article></div></section>';

		return $html;
	}

	/**
	 * Whether a field group uses volksimmobilien multi-section HTML import.
	 *
	 * @param array<string,mixed>|null $group Field group schema.
	 * @return bool
	 */
	public static function is_volks_multi_section_group( $group ) {
		if ( ! is_array( $group ) ) {
			return false;
		}

		return 'volks_multi_section' === (string) ( $group['import_mode'] ?? '' );
	}

	/**
	 * Build admin summary for one volks HTML section row.
	 *
	 * @param string $section_id Section anchor id.
	 * @param string $html       Section HTML.
	 * @return array{title:string,section_id:string,classes:string,size_kb:string,excerpt:string}
	 */
	public static function get_volks_section_admin_meta( $section_id, $html ) {
		$section_id = sanitize_html_class( (string) $section_id );
		$html       = (string) $html;
		$title      = '';
		$classes    = '';

		if ( preg_match( '/<section[^>]*\sid=(["\'])([^"\']+)\1/i', $html, $match ) && '' === $section_id ) {
			$section_id = sanitize_html_class( (string) $match[2] );
		}

		if ( preg_match( '/<section[^>]*\sclass=(["\'])([^"\']+)\1/i', $html, $match ) ) {
			$classes = trim( (string) $match[2] );
		}

		if ( preg_match( '/<h1[^>]*>(.*?)<\/h1>/is', $html, $match ) ) {
			$title = trim( wp_strip_all_tags( (string) $match[1] ) );
		} elseif ( preg_match( '/<h2[^>]*>(.*?)<\/h2>/is', $html, $match ) ) {
			$title = trim( wp_strip_all_tags( (string) $match[1] ) );
		} elseif ( preg_match( '/<h3[^>]*>(.*?)<\/h3>/is', $html, $match ) ) {
			$title = trim( wp_strip_all_tags( (string) $match[1] ) );
		}
		...
		if ( function_exists( 'mb_substr' ) && function_exists( 'mb_strlen' ) ) {
			$excerpt = mb_strlen( $plain ) > 140 ? mb_substr( $plain, 0, 137 ) . '…' : $plain;
		} else {
			$excerpt = strlen( $plain ) > 140 ? substr( $plain, 0, 137 ) . '…' : $plain;
		}
		...
		return array(
			...
			'classes'    => $classes,
			'size_kb'    => number_format( strlen( $html ) / 1024, 1, ',', '.' ) . ' KB',
			'excerpt'    => $excerpt,
		);
	}
	 ...
	 */
	/**
	 * volksimmobilien: eine importierte HTML-Sektion.
	 *
	 * @return array<string,mixed>
	 */
	protected static function layout_volks_html_section() {
		return array(
			'label'    => 'HTML-Sektion',
			'template' => 'volks_html_section',
			'fields'   => array(
				'section_id' => self::text( 'Section-ID (Anker)' ),
				'html'       => self::editor( 'Sektions-HTML' ),
			),
		);
	}

	/**
	 * Public accessor for volks HTML fallback layout.
	 *
	 * @return array<string,mixed>
	 */
	public static function get_layout_volks_html_section() {
		return self::layout_volks_html_section();
	}

	/**
	 * Admin summary for structured volks section rows.
	 *
	 * @param string               $layout  Layout key.
	 * @param array<string,mixed>  $section Section data.
	 * @param array<string,mixed>|null $schema Layout schema.
	 * @return array{title:string,section_id:string,classes:string,size_kb:string,excerpt:string}
	 */
	public static function get_volks_structured_section_admin_meta( $layout, $section, $schema = null ) {
		$section   = is_array( $section ) ? $section : array();
		$schema    = is_array( $schema ) ? $schema : array();
		$layout_label = (string) ( $schema['label'] ?? ucfirst( str_replace( '_', ' ', (string) $layout ) ) );
		$section_id = sanitize_html_class( (string) ( $section['section_id'] ?? '' ) );

		$title_keys = array( 'admin_label', 'title', 'claim', 'headline' );
		$title      = '';
		foreach ( $title_keys as $key ) {
			if ( ! empty( $section[ $key ] ) ) {
				$title = trim( wp_strip_all_tags( (string) $section[ $key ] ) );
				break;
			}
		}
		if ( '' === $title ) {
		...
		$excerpt_keys = array( 'subtitle', 'lead', 'text', 'paragraph_1', 'body' );
		$excerpt      = '';
		foreach ( $excerpt_keys as $key ) {
			if ( ! empty( $section[ $key ] ) ) {
				$excerpt = trim( preg_replace( '/\s+/u', ' ', wp_strip_all_tags( (string) $section[ $key ] ) ) );
				break;
			}
		}
		if ( '' === $excerpt && ! empty( $section['paragraphs'] ) && is_array( $section['paragraphs'] ) ) {
			$first_paragraph = reset( $section['paragraphs'] );
			if ( is_array( $first_paragraph ) && ! empty( $first_paragraph['text'] ) ) {
				$excerpt = trim( preg_replace( '/\s+/u', ' ', wp_strip_all_tags( (string) $first_paragraph['text'] ) ) );
			}
		}
		if ( function_exists( 'mb_substr' ) && function_exists( 'mb_strlen' ) ) {
			$excerpt = mb_strlen( $excerpt ) > 140 ? mb_substr( $excerpt, 0, 137 ) . '…' : $excerpt;
