 ...
 * Description: Import static HTML (volksimmobilien / ACM) into WordPress pages, media and Leadwerk Fields.
 * Version: 1.0.0
 * Author: Leadwerk
 * Text Domain: leadwerk-importer
 * Requires at least: 6.0
 * Requires PHP: 7.4
 * Requires Plugins: leadwerk-fields
 *
...
define( 'LEADWERK_IMPORT_MANIFEST_FILE', 'mapping-volks.json' );

require_once LEADWERK_IMPORTER_PATH . 'includes/class-leadwerk-news-cpt.php';
require_once LEADWERK_IMPORTER_PATH . 'includes/class-leadwerk-volks-section-parser.php';
...
function leadwerk_importer_resolve_wpml_clone_path() {
	...
	foreach ( array( $parent . '/leadwerk-wpml-clone', $parent . '/leadwerk_wpml_clone' ) as $path ) {
		if (
			is_dir( $path )
			&& is_file( $path . '/includes/class-leadwerk-translation-api.php' )
			&& is_file( $path . '/includes/class-leadwerk-html-segments.php' )
			&& is_file( $path . '/includes/class-leadwerk-translation-sync.php' )
		) {
			return $path;
		}
	}
...
require_once LEADWERK_IMPORTER_PATH . 'includes/class-leadwerk-volks-media.php';
...
function leadwerk_importer_admin_dependency_notices() {
	...
	$volks_manifest = defined( 'LEADWERK_IMPORT_MANIFEST_FILE' ) && 'mapping-volks.json' === LEADWERK_IMPORT_MANIFEST_FILE;
	if ( ! $volks_manifest && ( '' === $wpml_main || ! is_file( $wpml_main ) ) ) {
...
function leadwerk_importer_post_import_admin_notice() {
	...
	$is_volks    = defined( 'LEADWERK_IMPORT_MANIFEST_FILE' ) && 'mapping-volks.json' === LEADWERK_IMPORT_MANIFEST_FILE;

	echo '<div class="notice notice-warning is-dismissible"><p><strong>Leadwerk Importer:</strong> ';
	if ( $is_volks ) {
		echo esc_html__( 'Ein Live-Import wurde abgeschlossen. Bitte Permalinks speichern, die Startseite unter Lesen prüfen und Startseite, Unterseiten, Danke-Seite, 404-Seite sowie verlinkte Medien stichprobenartig oeffnen. Im Import-Live-Log Warnungen zu Medien oder Flexible Layouts prüfen.', 'leadwerk-importer' );
	} else {
...
function leadwerk_importer_admin_page() {
	...
	<div class="wrap leadwerk-importer-admin">
		...
		<p>volksimmobilien: statische HTML-Seiten, Medien und Leadwerk Fields importieren (Manifest <code>mapping-volks.json</code>). Theme: <code>volksimmobilien_theme</code>. Kein WPML Clone noetig.</p>
		<details class="leadwerk-importer-checklist" style="margin:12px 0;padding:12px 14px;background:#f6f7f7;border:1px solid #c3c4c7;border-radius:4px;max-width:920px;">
			...
			<ol style="margin:10px 0 0 1.2em;line-height:1.55;">
				<li>Plugins <code>leadwerk-fields</code> und <code>leadwerk_importer</code> aktivieren. Theme <code>volksimmobilien_theme</code>. Quelle: <code>leadwerk_importer/source_assets</code> (per <code>scripts/sync-volks-source-assets.sh</code> aus dem Repo-Root). Siehe <code>VOLKS-DEPLOY.md</code>.</li>
				<li><strong>Aktivierung:</strong> <code>leadwerk-fields</code>, dann <code>leadwerk_importer</code>. ACF deaktiviert lassen.</li>
				...
				<li>Struktur im Repo prüfen: <code>php scripts/verify-leadwerk-deployment.php</code> (Projektroot) — prüft u. a. HTML-Dateien, Drift, und ob jede <code>field_name</code> aus <code>mapping-volks.json</code> in <code>Leadwerk_Content_Schema</code> existiert. Optional strikt: <code>--strict-drift</code>. HTML spiegeln (Kanon = <code>leadwerk_importer/source_assets</code>): <code>php scripts/sync-html-sources.php</code>. Abgeleitetes <code>import-manifest.json</code>: <code>php scripts/build-import-manifest.php</code> (Importer liest <code>mapping-volks.json</code>).</li>
		...
		<details class="leadwerk-importer-fallback">
			...
			<p>
				<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( array( 'run' => '1', 'dry_run' => '1' ), admin_url( 'tools.php?page=leadwerk-import' ) ), 'leadwerk_import_run' ) ); ?>" class="button">Legacy Dry-Run</a>
				<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( array( 'run' => '1' ), admin_url( 'tools.php?page=leadwerk-import' ) ), 'leadwerk_import_run' ) ); ?>" class="button">Legacy Import</a>
			</p>
		</details>
...
/**
 * Repair stored volks media URLs without a full re-import.
 *
 * @return void
 */
function leadwerk_importer_ajax_repair_volks_media() {
	leadwerk_importer_verify_ajax_request();

	$importer = new Leadwerk_Importer( false );
	$report   = $importer->repair_volks_stored_media_urls();

	wp_send_json_success(
		array(
			'message' => sprintf(
				'Medien-URLs repariert: %d aktualisiert, %d unveraendert.',
...
add_action( 'wp_ajax_leadwerk_repair_volks_media', 'leadwerk_importer_ajax_repair_volks_media' );
