...
class Leadwerk_Importer {
	...
	public function __construct( $apply = false ) {
		...
		$repo_root = dirname( dirname( LEADWERK_IMPORTER_PATH ) );
		if ( is_dir( $repo_root ) && is_file( $repo_root . '/index.html' ) && is_dir( $repo_root . '/Fotos' ) ) {
			$this->source_root = $repo_root;
		}
	...
	protected function load_manifest() {
		$manifest_file = (string) apply_filters( 'leadwerk_import_manifest_file', '' );
		if ( '' === $manifest_file ) {
			$manifest_file = defined( 'LEADWERK_IMPORT_MANIFEST_FILE' ) ? (string) LEADWERK_IMPORT_MANIFEST_FILE : 'mapping-volks.json';
		}

		$candidates = array( $manifest_file );
		if ( 'mapping-volks.json' !== $manifest_file ) {
			$candidates[] = 'mapping-volks.json';
		}
		$candidates[] = 'mapping.json';

		$path = '';
		foreach ( array_unique( $candidates ) as $candidate ) {
			$candidate_path = $this->manifest_dir . ltrim( $candidate, '/' );
		...
		if ( '' === $path ) {
			Leadwerk_Logger::log( 'Manifest nicht gefunden in ' . $this->manifest_dir, 'error' );
		...
		Leadwerk_Logger::log(
			sprintf(
				'Manifest geladen (%s): %d Seiten',
				basename( $path ),
				count( $this->manifest['pages'] ?? array() )
			)
		);
	}

	/**
	 * Whether DE/EN translation posts should be created for this manifest.
	 *
	 * @return bool
	 */
	protected function translations_enabled() {
		if ( isset( $this->manifest['translations_enabled'] ) ) {
			return (bool) $this->manifest['translations_enabled'];
		}

		return 'volksimmobilien' !== sanitize_key( (string) ( $this->manifest['project'] ?? '' ) );
	}

	/**
	 * Active manifest project slug.
	 *
	 * @return string
	 */
	protected function get_import_project() {
		return sanitize_key( (string) ( $this->manifest['project'] ?? 'acm' ) );
	}
	...
	public function repair_current_structured_content( $source_keys = array() ) {
		...
		if ( ! function_exists( 'get_field' ) || ! function_exists( 'update_field' ) ) {
			return new WP_Error( 'leadwerk_repair_missing_acf', 'ACF Field Funktionen sind fuer den Repair-Run nicht verfuegbar.' );
		}

		if ( ! function_exists( 'leadwerk_theme_render_exact_page_group' ) && 'volksimmobilien' !== $this->get_import_project() ) {
		...
		foreach ( array_values( $page_configs ) as $index => $page_config ) {
			...
			$current_value       = get_field( $field_name, $de_id );
			...
			if ( empty( $candidate['is_valid'] ) ) {
				$page_entry['status']  = 'error';
				$page_entry['message'] = 'Weder Snapshot noch Shell-Fallback konnte valide Exact-HTML liefern.';
				$report['failed']++;
				$report['pages'][ $source_key ] = $page_entry;
				Leadwerk_Logger::record_result( 'error', 'Structured repair fehlgeschlagen fuer ' . $source_key . ': kein valider Snapshot/Shell-Kandidat.', $source_key . '-repair' );
				Leadwerk_Logger::update_step( 'repair_structured', array( 'processed' => $index + 1 ) );
				continue;
			}
			...
			update_field( $field_name, $candidate['value'], $de_id );
			...
			if ( ! empty( $readback_validation['has_visible_content'] ) && ! empty( $readback_render['is_valid'] ) ) {
				...
				update_field( $field_name, $current_value, $de_id );
				$page_entry['status']  = 'error';
	...
	/**
	 * Render one structured field group to HTML for validation.
	 *
	 * @param array<string,mixed> $group   Group schema.
	 ...
	 * @return string
	 */
	protected function render_exact_page_group( $group, $value, $post_id ) {
		if ( function_exists( 'leadwerk_theme_render_exact_page_group' ) ) {
			return (string) leadwerk_theme_render_exact_page_group( $group, $value, (int) $post_id );
		}

		return $this->render_volks_exact_page_group_fallback( $group, $value );
	}

	/**
	 * Volksimmobilien fallback renderer when the theme function is unavailable.
	 *
	 * @param array<string,mixed> $group Field group schema.
	 * @param mixed               $value Stored value.
	 * @return string
	 */
	protected function render_volks_exact_page_group_fallback( $group, $value ) {
		if ( 'volksimmobilien' !== $this->get_import_project() || ! is_array( $value ) ) {
			return '';
		}
		...
		if ( empty( $group['layouts'] ) ) {
			...
			if ( class_exists( 'Leadwerk_Volks_Media' ) ) {
				$content = Leadwerk_Volks_Media::rewrite_html_fragment( $content );
			}

			$html = '<section class="legal-content-section"><article class="legal-prose">';
			if ( '' !== trim( $headline ) ) {
				$html .= '<h1>' . esc_html( $headline ) . '</h1>';
			}
		...
		foreach ( $value as $section ) {
			...
			} elseif ( 'volks_editable_html_section' === $layout ) {
				$section_html = (string) ( $section['original_html'] ?? '' );
			}
			if ( '' === trim( $section_html ) ) {
				continue;
			}
			if ( class_exists( 'Leadwerk_Volks_Media' ) ) {
				$section_html = Leadwerk_Volks_Media::rewrite_html_fragment( $section_html );
			}
			$html .= $section_html;
		}
	...
	protected function validate_exact_group_render( $group, $value, $page_config, $post_id ) {
		...
		$html                       = $this->render_exact_page_group( $render_group, $value, (int) $post_id );
	...
	protected function perform_preflight_step( $job_state ) {
		...
		if ( $this->translations_enabled() && ! class_exists( 'Leadwerk_Translation_API' ) ) {
			$blocking[] = 'Leadwerk WPML Clone API ist nicht geladen (fuer EN-Uebersetzungen erforderlich).';
		}
	...
	protected function record_preflight_source_asset_warnings() {
		...
		if ( 'volksimmobilien' === $this->get_import_project() ) {
			$volks_logo = $root . DIRECTORY_SEPARATOR . 'Fotos' . DIRECTORY_SEPARATOR . 'volksimmobilien-logo-weiss.webp';
			if ( ! is_file( $volks_logo ) ) {
				Leadwerk_Logger::record_result(
					'warning',
					'Fotos/volksimmobilien-logo-weiss.webp fehlt — Logo im Theme nutzt Fallback.',
					'preflight'
				);
			}
			return;
	...
	protected function perform_page_upsert_step( $job_state, $batch_size ) {
		...
		while ( $cursor < $total && $processed < $batch_size ) {
			...
			$en_result = $this->translations_enabled()
				? $this->upsert_page( $page_config, 'en' )
				: array(
					'post_id'     => 0,
					'lang'        => 'en',
					'created_new' => false,
			...
			if ( $this->translations_enabled() && ! $this->dry_run && ! empty( $de_result['post_id'] ) && ! empty( $en_result['post_id'] ) && class_exists( 'Leadwerk_Translation_API' ) ) {
			...
			if ( ! empty( $de_result['post_id'] ) && ( ! $this->translations_enabled() || ! empty( $en_result['post_id'] ) ) ) {
	...
	protected function perform_page_fill_de_step( $job_state, $batch_size ) {
		...
		while ( $cursor < $total && $processed < $batch_size ) {
			...
			if ( $de_id && is_array( $group ) && ! empty( $group['layouts'] ) && function_exists( 'get_field' ) ) {
				$expected_layouts = count( (array) $group['layouts'] );
				$acf_rows         = get_field( $field_name, $de_id );
				$row_count        = is_array( $acf_rows ) ? count( array_values( $acf_rows ) ) : 0;
				if ( $row_count > 0 && $row_count !== $expected_layouts ) {
					Leadwerk_Logger::log(
						sprintf(
							'ACF/Shell-Hinweis %s: %d Flexible-Layouts in der DB, Schema erwartet %d (Exact-Renderer pairt nach Index!).',
							$source_key,
							$row_count,
							$expected_layouts
						),
			...
			$previous            = function_exists( 'get_field' ) && $de_id ? get_field( $field_name, $de_id ) : null;
			...
			if ( $payload_has_data && ! empty( $write_guard['skip_write'] ) ) {
				...
				if ( ! $this->dry_run ) {
					update_field( $field_name, $payload['value'], $de_id );
				...
				if ( ! empty( $readback_validation['has_visible_content'] ) ) {
					...
					if ( ! $this->dry_run && ! empty( $previous_validation['has_visible_content'] ) ) {
						update_field( $field_name, $previous, $de_id );
						$readback            = get_field( $field_name, $de_id );
						$readback_validation = $filler->validate_group_value( $field_name, $readback );
	...
	protected function perform_force_canonical_sync_step( $job_state, $batch_size ) {
		...
		while ( $cursor < $total && $processed < $batch_size ) {
			...
			$group['field_name'] = $field_name;
			$previous            = function_exists( 'get_field' ) ? get_field( $field_name, $de_id ) : null;
			$previous_validation = $filler->validate_group_value( $field_name, $previous );
			...
			if ( empty( $validation['has_visible_content'] ) || empty( $payload_render['is_valid'] ) ) {
				...
				update_field( $field_name, $payload_value, $de_id );
				$readback            = get_field( $field_name, $de_id );
				$readback_validation = $filler->validate_group_value( $field_name, $readback );
				$readback_render     = $this->validate_exact_group_render( $group, $readback, $page_config, $de_id );
	...
	protected function apply_page_meta( $post_id, $payload, $page_config, $lang ) {
		...
		$this->apply_volks_open_graph_meta( $post_id, $payload, $page_config );
		$this->apply_volks_structured_data_meta( $post_id, $payload, $page_config );
	...
	/**
	 * Store Open Graph / Twitter preview image for link shares (Yoast + fallback meta).
	 *
	 * @param int                 $post_id     Post ID.
	 * @param array<string,mixed> $payload     Import payload.
	 * @param array<string,mixed> $page_config Page manifest row.
	 * @return void
	 */
	protected function apply_volks_open_graph_meta( $post_id, $payload, $page_config ) {
		if ( 'volksimmobilien' !== $this->get_import_project() ) {
			return;
		}
		...
		$source_key = sanitize_key( (string) ( $page_config['source_key'] ?? '' ) );
		$og_html    = trim( (string) ( $payload['og_image'] ?? '' ) );

		if ( function_exists( 'volks_resolve_og_image_path_for_import' ) ) {
			$relative_path = volks_resolve_og_image_path_for_import( $source_key, $og_html );
		} else {
			$relative_path = $og_html;
		}
		...
		update_post_meta( $post_id, 'leadwerk_og_image_path', sanitize_text_field( $relative_path ) );

		$attachment_id = $this->resolve_attachment( $relative_path );
		$image_url   = '';

		if ( $attachment_id > 0 ) {
			$image_url = (string) wp_get_attachment_url( $attachment_id );
		} elseif ( function_exists( 'volks_resolve_og_image_url' ) ) {
			$image_url = volks_resolve_og_image_url( $relative_path );
		}
		...
		if ( '' !== $image_url ) {
			...
			update_post_meta( $post_id, '_yoast_wpseo_twitter-image', esc_url_raw( $image_url ) );
		}

		$og_title = trim( (string) ( $payload['og_title'] ?? '' ) );
		if ( '' === $og_title ) {
			$og_title = trim( (string) ( $payload['document_title'] ?? '' ) );
		}
		if ( '' !== $og_title ) {
			update_post_meta( $post_id, '_yoast_wpseo_opengraph-title', sanitize_text_field( $og_title ) );
			update_post_meta( $post_id, '_yoast_wpseo_twitter-title', sanitize_text_field( $og_title ) );
		}

		$og_desc = trim( (string) ( $payload['meta_description'] ?? '' ) );
		if ( '' !== $og_desc ) {
			update_post_meta( $post_id, '_yoast_wpseo_opengraph-description', sanitize_text_field( $og_desc ) );
			update_post_meta( $post_id, '_yoast_wpseo_twitter-description', sanitize_text_field( $og_desc ) );
		}

		update_post_meta( $post_id, '_yoast_wpseo_twitter-card-type', 'summary_large_image' );
	}

	/**
	 * Persist FAQPage / RealEstateAgent JSON-LD from static HTML for theme output.
	 *
	 * @param int                 $post_id     Post ID.
	 * @param array<string,mixed> $payload     Import payload.
	 * @param array<string,mixed> $page_config Page manifest row.
	 * @return void
	 */
	protected function apply_volks_structured_data_meta( $post_id, $payload, $page_config ) {
		if ( 'volksimmobilien' !== $this->get_import_project() ) {
			return;
		}
		...
		$documents = array();
		if ( ! empty( $payload['json_ld'] ) && is_array( $payload['json_ld'] ) ) {
			$documents = $payload['json_ld'];
		}
		...
		foreach ( $documents as $document ) {
			...
			if ( 'FAQPage' === $type && null === $faq_schema ) {
				$faq_schema = $document;
			}
			if ( 'RealEstateAgent' === $type && null === $agent_schema ) {
				$agent_schema = $document;
			}
		}

		if ( is_array( $faq_schema ) && ! empty( $faq_schema['mainEntity'] ) ) {
			$json = wp_json_encode( $faq_schema, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES );
			if ( is_string( $json ) && '' !== $json ) {
				update_post_meta( $post_id, 'leadwerk_faq_schema_json', $json );
			}
		} else {
			delete_post_meta( $post_id, 'leadwerk_faq_schema_json' );
		}

		if ( is_array( $agent_schema ) ) {
			$json = wp_json_encode( $agent_schema, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES );
			if ( is_string( $json ) && '' !== $json ) {
				update_post_meta( $post_id, 'leadwerk_realestate_schema_json', $json );
			}
		} else {
			delete_post_meta( $post_id, 'leadwerk_realestate_schema_json' );
		}
	}

	/**
	 * Refresh Yoast indexables after meta import.
	 *
	 * @param int $post_id Post ID.
	 * @return void
	 */
	...
	/**
	 * Fill Leadwerk options for volksimmobilien (minimal set).
	 *
	 * @return void
	 */
	protected function fill_volks_options() {
		if ( ! $this->leadwerk_get_option( 'company_address' ) ) {
			$this->leadwerk_update_option(
				'company_address',
		...
		if ( ! $this->leadwerk_get_option( 'company_phone' ) ) {
			$this->leadwerk_update_option( 'company_phone', '+49 170 2 98 51 41' );
		}
	...
	/**
	 * Rewrite stored volks HTML media URLs to attachment/source URLs.
	 *
	 * @return array{updated:int,skipped:int}
	 */
	public function repair_volks_stored_media_urls() {
		$report = array(
			'updated' => 0,
			'skipped' => 0,
		);

		if ( 'volksimmobilien' !== $this->get_import_project() || ! class_exists( 'Leadwerk_Volks_Media' ) ) {
			return $report;
		}

		$filler = $this->get_filler();
		if ( ! $filler instanceof Leadwerk_ACF_Filler ) {
			return $report;
		}
		...
		foreach ( (array) ( $this->manifest['pages'] ?? array() ) as $page_config ) {
			...
			$post_id = (int) $this->find_page_by_source_key_and_lang( $source_key, 'de', (string) ( $page_config['target_type'] ?? 'page' ) );
			if ( $post_id <= 0 ) {
				$report['skipped']++;
				continue;
			}

			if ( $filler->repair_field_media_urls( $post_id, $field_name ) ) {
				$report['updated']++;
			} else {
				$report['skipped']++;
			}
		...
		if ( $report['updated'] > 0 ) {
			Leadwerk_Logger::record_result(
				'success',
				sprintf( 'Volks-Medien-URLs in %d Feldgruppe(n) aktualisiert.', (int) $report['updated'] ),
				'volks-media-repair'
			);
		}
	...
	protected function is_legal_page( $page_config ) {
		...
		if ( class_exists( 'Leadwerk_Content_Schema' ) ) {
			$group = Leadwerk_Content_Schema::get_group( $field_name );
			if ( is_array( $group ) && empty( $group['layouts'] ) && ! empty( $group['sync_post_content'] ) ) {
				return true;
			}
		}

		return in_array( $field_name, array( 'impressum_page', 'datenschutz_page' ), true );
	}
	...
	/**
	 * Meta key aligned with theme LEADWERK_THEME_ACM_NEWS_PUBLICATION_DATE_META (Leadwerk Fields / get_field).
	 *
	 * @return string
	 */
	protected function leadwerk_importer_acm_news_publication_meta_key() {
		return 'acm_news_publication_date';
	}

	/**
	 * Strip related-news teaser cards so their &lt;time&gt; does not override the article date.
	 *
	 * @param string $html Full HTML.
	 * @return string
	 */
	protected function scope_html_for_news_article_datetime( $html ) {
		$html = (string) $html;
		return (string) preg_replace( '/<article[^>]*\bnews-card-related\b[^>]*>.*?<\/article>/is', '', $html );
	}

	/**
	 * Normalize extracted date string to Y-m-d for post meta and sorting.
	 *
	 * @param string $raw Raw from &lt;time datetime&gt; or visible label.
	 * @return string Empty if not parseable.
	 */
	protected function normalize_acm_news_publication_ymd( $raw ) {
		$raw = trim( (string) $raw );
		if ( '' === $raw ) {
			return '';
		}
		if ( preg_match( '/^\d{4}-\d{2}-\d{2}/', $raw ) ) {
			...
			$ts   = strtotime( $head . ' 12:00:00' );
			if ( $ts ) {
				return function_exists( 'wp_date' ) ? wp_date( 'Y-m-d', $ts ) : gmdate( 'Y-m-d', $ts );
			}
		}
		if ( preg_match( '/^(\d{1,2})\.(\d{1,2})\.(\d{4})$/', $raw, $m ) ) {
			$ts = strtotime( sprintf( '%04d-%02d-%02d', (int) $m[3], (int) $m[2], (int) $m[1] ) . ' 12:00:00' );
			if ( $ts ) {
				return function_exists( 'wp_date' ) ? wp_date( 'Y-m-d', $ts ) : gmdate( 'Y-m-d', $ts );
			}
		}
		$parsed = $this->parse_german_long_month_news_date( $raw );
		if ( '' !== $parsed ) {
			return $parsed;
		}
		$ts = strtotime( $raw );
	...
	/**
	 * Parse labels like "1. Juni 2025" (German month names).
	 *
	 * @param string $label Plain text.
	 * @return string Y-m-d or empty.
	 */
	protected function parse_german_long_month_news_date( $label ) {
		...
		if ( ! preg_match( '/^(\d{1,2})\.\s*([^\d\s.]+)\s+(\d{4})$/u', $label, $m ) ) {
			return '';
		}
		$day        = (int) $m[1];
		$month_word = strtolower( str_replace( array( "\xc3\xa4", "\xc3\xb6", "\xc3\xbc" ), array( 'a', 'o', 'u' ), $m[2] ) );
		$month_word = str_replace( array( 'ä', 'ö', 'ü', 'ß' ), array( 'a', 'o', 'u', 'ss' ), $month_word );
		$year       = (int) $m[3];
		if ( ! isset( $months[ $month_word ] ) ) {
			return '';
		}
		$month = (int) $months[ $month_word ];
		$ts    = strtotime( sprintf( '%04d-%02d-%02d', $year, $month, $day ) . ' 12:00:00' );
		if ( ! $ts ) {
			return '';
		}
		return function_exists( 'wp_date' ) ? wp_date( 'Y-m-d', $ts ) : gmdate( 'Y-m-d', $ts );
	...
	/**
	 * Write canonical publication date meta (_leadwerk_news_datetime + Leadwerk CF key).
	 *
	 * @param int    $post_id       Post ID.
	 * @param string $raw_extracted Raw from HTML (datetime attribute or fallback text).
	 * @return void
	 */
	protected function persist_acm_news_publication_metas( $post_id, $raw_extracted ) {
		$post_id = (int) $post_id;
		if ( $post_id <= 0 || $this->dry_run ) {
			return;
		}
		$ymd = $this->normalize_acm_news_publication_ymd( $raw_extracted );
		if ( '' === $ymd ) {
			return;
		}
		update_post_meta( $post_id, '_leadwerk_news_datetime', $ymd );
		update_post_meta( $post_id, $this->leadwerk_importer_acm_news_publication_meta_key(), $ymd );
	}

	/**
	 * Article date: first &lt;time datetime&gt; outside related-news cards; optional .news-date span.
	 *
	 * @param string $html Full HTML.
	 * @return string Raw token for normalize_acm_news_publication_ymd().
	 */
	protected function extract_news_datetime_from_html( $html ) {
		$scope = $this->scope_html_for_news_article_datetime( $html );
		if ( preg_match( '/<time[^>]*\bdatetime\s*=\s*["\']([^"\']+)["\']/', $scope, $m ) ) {
			return sanitize_text_field( $m[1] );
		}
		if ( preg_match( '/<span[^>]*\bnews-date\b[^>]*>([^<]+)<\/span>/i', $scope, $m2 ) ) {
			return sanitize_text_field( trim( html_entity_decode( $m2[1], ENT_QUOTES | ENT_HTML5, 'UTF-8' ) ) );
		}
		return '';
	}
		...
		while ( $cursor < $total && $processed < $batch ) {
			...
			if ( $this->dry_run ) {
				...
				if ( $existing->have_posts() ) {
					...
					if ( '' !== $news_datetime ) {
						$this->persist_acm_news_publication_metas( $post_id, $news_datetime );
					}
