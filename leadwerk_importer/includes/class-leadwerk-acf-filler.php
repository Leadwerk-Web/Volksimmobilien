...
class Leadwerk_ACF_Filler {
	 ...
	 * @param string                        $import_project Manifest project slug.
	 */
	public function __construct( $source_root = '', $media_importer = null, $page_lookup = array(), $import_project = '' ) {
		...
		$this->import_project  = sanitize_key( (string) $import_project );
	}

	/**
	 * Whether the active manifest targets volksimmobilien.
	 *
	 * @return bool
	 */
	protected function is_volks_import_project() {
		return 'volksimmobilien' === $this->import_project;
	}
	...
	public function build_page_payload_from_html( $page_config, $html, $lang = 'de' ) {
		...
		$payload = array(
			...
			'og_image'         => $this->attr( $xpath, '//meta[@property="og:image"]', 'content' ),
			'og_title'         => $this->attr( $xpath, '//meta[@property="og:title"]', 'content' ),
			'json_ld'          => $this->extract_json_ld_documents( $html ),
			'value'            => array(),
			'validation'       => array(),
		...
		if ( ! empty( $group['import_mode'] ) && 'volks_multi_section' === (string) $group['import_mode'] ) {
			return $this->build_volks_multi_section_payload( $page_config, $group, $xpath, $html );
		}
	...
	protected function resolve_legal_match( $xpath ) {
		...
		if ( ! $section instanceof DOMNode ) {
			$section = $this->first_node( $xpath, '//article[contains(@class,"legal-prose")][1]' );
		}
		if ( ! $section instanceof DOMNode ) {
			$section = $this->first_node( $xpath, '//section[contains(@class,"legal-content-section")][1]' );
		}

		return array(
			'node'          => $section instanceof DOMNode ? $section : null,
			'matched_by'    => $section instanceof DOMNode ? 'selector_fallback' : 'missing',
			'selector_used' => '.legal-content, .legal-prose',
	...
	protected function build_legal_value( $xpath, $section_node = null ) {
		...
		if ( '' === trim( $content ) ) {
			$fallback_query = $section_node instanceof DOMNode
				? './/*[contains(@class,"legal-body")][1] | .//*[contains(@class,"legal-copy")][1] | .//*[contains(@class,"legal-prose")][1]'
				: '//section[contains(@class,"legal-content")]//*[contains(@class,"legal-body")][1] | //section[contains(@class,"legal-content")]//*[contains(@class,"legal-copy")][1] | //article[contains(@class,"legal-prose")][1]';
			$fallback_node = $this->first_node( $context, $fallback_query );
			if ( $fallback_node instanceof DOMElement && false !== strpos( ' ' . $fallback_node->getAttribute( 'class' ) . ' ', ' legal-prose ' ) ) {
				$content = $this->extract_legal_prose_body_html( $fallback_node );
			} elseif ( $fallback_node instanceof DOMNode ) {
				$content = $this->save_inner_html( $fallback_node );
			}
		}

		$headline_query = $section_node instanceof DOMNode
			? './/*[contains(@class,"legal-title")][1] | .//*[contains(@class,"legal-prose__title")][1] | .//h1[1] | .//h2[1]'
			: '//section[contains(@class,"legal-content")]//*[contains(@class,"legal-title")][1] | //section[contains(@class,"legal-content")]//h1[1]';

		$content = $this->rewrite_volks_html_links( $content );
	...
	/**
	 * Extract legal-prose body HTML without the page title (stored in headline).
	 *
	 * @param DOMElement $prose_node Article.legal-prose node.
	 * @return string
	 */
	protected function extract_legal_prose_body_html( DOMElement $prose_node ) {
		$clone = $prose_node->cloneNode( true );
		if ( ! $clone instanceof DOMElement ) {
			return $this->save_inner_html( $prose_node );
		}

		$xpath = new DOMXPath( $prose_node->ownerDocument );
		$nodes = $xpath->query( './/*[contains(@class,"legal-prose__title")]', $clone );
		...
		return trim( $this->save_inner_html( $clone ) );
	}
	...
	protected function parse_layout_value( $layout_key, $layout_schema, $section_node, $lang ) {
		...
		switch ( (string) ( $layout_schema['template'] ?? $layout_key ) ) {
			...
			case 'volks_html_section':
				return $this->parse_volks_html_section( $section_node );
	...
	/**
	 * Build payload for volksimmobilien multi-section groups.
	 *
	 * @param array<string,mixed> $page_config Page config.
	 ...
	 * @return array<string,mixed>
	 */
	protected function build_volks_multi_section_payload( $page_config, $group, $xpath, $html = '' ) {
		$field_name         = (string) $page_config['field_name'];
		$use_structured     = class_exists( 'Leadwerk_Volks_Section_Parser' );
		$sections           = $this->extract_body_sections( $xpath );
		$values             = array();
		$parser             = $use_structured ? new Leadwerk_Volks_Section_Parser( $this ) : null;
		...
		foreach ( $sections as $section_node ) {
			...
			if ( $parser ) {
				if ( 'volks_home_sections' === $field_name ) {
					$parsed = $parser->parse_home_section( $section_node );
				} elseif ( class_exists( 'Leadwerk_Volks_Section_Schema' ) && Leadwerk_Volks_Section_Schema::landing_field_preserves_full_html( $field_name ) ) {
					// Mallorca/Kaufen/Verkaufen/Bewerten: komplexes Markup (Slider, Reels, Editorial) als Ganzes.
					$parsed = $parser->parse_editable_html_section( $section_node );
				} else {
					$parsed = $parser->parse_landing_section( $section_node );
				}
				$layout_key = (string) ( $parsed['acf_fc_layout'] ?? 'html_section' );
				$values[]   = $this->normalize_layout_value( $field_name, $layout_key, $parsed );
				continue;
			}
			...
			$values[] = $this->normalize_layout_value(
				$field_name,
				'html_section',
				$this->parse_volks_html_section( $section_node )
			);
		}
		...
		$payload = array(
			...
			'og_image'           => $this->attr( $xpath, '//meta[@property="og:image"]', 'content' ),
			'og_title'             => $this->attr( $xpath, '//meta[@property="og:title"]', 'content' ),
			'json_ld'              => $this->extract_json_ld_documents( (string) $html ),
			'value'              => $values,
			'validation'         => array(),
			'layout_diagnostics' => array(),
		);

		$payload['validation'] = $this->build_payload_validation(
			(string) $page_config['field_name'],
			$group,
			$payload['value'],
			count( $sections )
	...
	/**
	 * Parse one volks HTML section into stored fields.
	 *
	 * @param DOMNode $section_node Section node.
	 * @return array<string,mixed>
	 */
	protected function parse_volks_html_section( $section_node ) {
		$html = $section_node instanceof DOMNode ? $section_node->ownerDocument->saveHTML( $section_node ) : '';
		$html = $this->rewrite_volks_html_links( $html );
	...
	/**
	 * Rewrite static href/src paths in section HTML during import.
	 *
	 * @param string $html Section HTML.
	 * @return string
	 */
	protected function rewrite_volks_html_links( $html ) {
		$html = trim( (string) $html );
		if ( '' === $html || ! class_exists( 'DOMDocument' ) ) {
			return $html;
		}

		$wrapped = '<?xml encoding="utf-8" ?><html><body><div id="volks-import-root">' . $html . '</div></body></html>';
		...
		$root  = $xpath->query( '//*[@id="volks-import-root"]' )->item( 0 );
		...
		foreach ( $xpath->query( './/*[@href]', $root ) as $node ) {
			...
			if ( '' !== $page_key ) {
				$node->setAttribute( 'data-volks-page-key', $page_key );
				if ( preg_match( '/#(.+)$/', $href, $m ) ) {
					$node->setAttribute( 'data-volks-anchor', sanitize_html_class( (string) $m[1] ) );
				}
			}
		}
	...
	protected function extract_body_sections( $xpath ) {
		...
		// ACM HTML uses <body><main><section> hierarchy.
		// Legacy FINORA HTML used <body><section> directly.
		// Try <main> wrapper first, fall back to direct body children.
		$main_sections = $this->query_nodes( $xpath, '//body/main/section' );
	...
	protected function map_href_to_source_key( $href ) {
		...
		if ( preg_match( '#^([a-z0-9_-]+)/\.html$#i', $href, $slug_match ) ) {
			$href = strtolower( $slug_match[1] ) . '.html';
		}
		...
		if ( $this->is_volks_import_project() ) {
			if ( isset( $volks_map[ $href ] ) ) {
				return $volks_map[ $href ];
			}
		...
		$map = array(
			'index.html'                => 'acm-index-v1',
			'thats-acm.html'            => 'acm-thats-acm-v1',
			'charter.html'              => 'acm-charter-v1',
			'global-7500.html'          => 'acm-global-7500-v1',
			'global-6000.html'          => 'acm-global-6000-v1',
			'global-xrs.html'           => 'acm-global-xrs-v1',
			'aircraft-management.html'  => 'acm-aircraft-v1',
			'maintenance.html'          => 'acm-maintenance-v1',
			'karriere.html'             => 'acm-careers-v1',
			'kontakt.html'              => 'acm-contact-v1',
			'news.html'                 => 'acm-news-v1',
			'impressum.html'            => 'acm-impressum-v1',
			'datenschutz.html'          => 'acm-datenschutz-v1',
			'en/index.html'             => 'acm-index-v1',
			'en/thats-acm.html'         => 'acm-thats-acm-v1',
			'en/charter.html'           => 'acm-charter-v1',
			'en/global-7500.html'       => 'acm-global-7500-v1',
			'en/global-6000.html'       => 'acm-global-6000-v1',
			'en/global-xrs.html'        => 'acm-global-xrs-v1',
			'en/aircraft-management.html' => 'acm-aircraft-v1',
			'en/maintenance.html'       => 'acm-maintenance-v1',
			'en/karriere.html'          => 'acm-careers-v1',
			'en/kontakt.html'           => 'acm-contact-v1',
			'en/news.html'              => 'acm-news-v1',
			'en/impressum.html'         => 'acm-impressum-v1',
			'en/datenschutz.html'       => 'acm-datenschutz-v1',
		);
	...
	/**
	 * Resolve static HTML hrefs to WordPress permalinks during volks import.
	 *
	 * @param string $href Raw href from source HTML.
	 * @return string
	 */
	protected function resolve_volks_import_href( $href ) {
		$href = trim( html_entity_decode( (string) $href, ENT_QUOTES | ENT_HTML5, 'UTF-8' ) );
		if ( '' === $href || '#' === $href ) {
			return $href;
		}

		// Same-page anchors: resolve at render time (import has no page context).
		if ( preg_match( '/^#[^#]+$/', $href ) ) {
			return $href;
		}
		...
		if ( function_exists( 'volks_resolve_href' ) ) {
			return volks_resolve_href( $href );
		}
		...
		if ( isset( $file_map[ $normalized ] ) ) {
			$source_key = $file_map[ $normalized ];
		} else {
		...
		if ( '' === $source_key ) {
			if ( 0 === strpos( $fragment, '#' ) && '' === $normalized ) {
				return home_url( '/' ) . $fragment;
			}
			return $href . $fragment;
		}

		$url = $this->get_volks_import_page_url( $source_key, $normalized );
		if ( '' === $fragment ) {
			return $url;
		}
	...
	/**
	 * Permalink for one volks source key during import (uses page_lookup when available).
	 *
	 * @param string $source_key   Source key.
	 * @param string $source_file  Relative HTML file.
	 * @return string
	 */
	protected function get_volks_import_page_url( $source_key, $source_file ) {
		...
		if ( '' !== $source_file && isset( $de_lookup[ $source_file ] ) ) {
			$permalink = get_permalink( (int) $de_lookup[ $source_file ] );
			if ( is_string( $permalink ) && '' !== $permalink ) {
				return $permalink;
			}
		}

		$path = isset( $fallbacks[ $source_key ] ) ? $fallbacks[ $source_key ] : '/';
	...
	protected function resolve_asset_source_path( $raw ) {
		if ( class_exists( 'Leadwerk_Volks_Media' ) ) {
			return Leadwerk_Volks_Media::normalize_path( $raw );
		}
	...
	protected function get_attachment_id_by_source( $path ) {
		...
		$id = 0;
		if ( class_exists( 'Leadwerk_Volks_Media' ) ) {
			$id = (int) Leadwerk_Volks_Media::get_attachment_id( $path );
		}
	...
	protected function parse_acm_home_final_cta( $node ) {
		$actions = array();
		foreach ( $this->query_nodes( $node, './/div[contains(@class,"homepage-cta-grid")]//a' ) as $link ) {
		...
		return array(
			'title'   => $this->html( $node, './/h2[1]' ),
			'body'    => $this->html( $node, './/p[contains(@class,"text-stone-600")][1] | .//div[contains(@class,"max-w-2xl")]//p[1]' ),
			'actions' => $actions,
		);
	}
	...
	protected function parse_acm_horizontal_timeline( $node ) {
		...
		foreach ( $this->query_nodes( $node, './/article[contains(concat(\' \', normalize-space(@class), \' \'), \' htl-card \')] | .//*[contains(concat(\' \', normalize-space(@class), \' \'), \' htl-item \')] | .//*[contains(concat(\' \', normalize-space(@class), \' \'), \' timeline-item \')]' ) as $item ) {
	...
	protected function parse_acm_contact_dept_section( $node ) {
		...
		$profiles = array();
		// Nur aeussere Profilkarten; sonst matcht contains(@class,"profile-card") auch Wrap/Image (3x pro Person).
		foreach ( $this->query_nodes( $node, './/div[contains(concat(\' \', normalize-space(@class), \' \'), \' profile-card \')]' ) as $card ) {
			$img    = $this->parse_image_fields( $card, './/img[1]' );
	...
	/**
	 * Rewrite stored media URLs in one page field group.
	 *
	 * @param int    $post_id    Page ID.
	 * @param string $field_name Field group name.
	 * @return bool Whether the field value was updated.
	 */
	public function repair_field_media_urls( $post_id, $field_name ) {
		if ( ! class_exists( 'Leadwerk_Volks_Media' ) || ! function_exists( 'get_field' ) || ! function_exists( 'update_field' ) ) {
			return false;
		}
		...
		if ( is_array( $group ) && ! empty( $group['layouts'] ) ) {
			foreach ( $value as $index => $row ) {
				...
				if ( 'html_section' === $layout ) {
					...
					$fixed = Leadwerk_Volks_Media::rewrite_html_fragment( $html );
					if ( $fixed !== $html ) {
						$value[ $index ]['html'] = $fixed;
						$changed               = true;
					}
					continue;
				}
				if ( 'volks_editable_html_section' === $layout ) {
					foreach ( array( 'original_html', 'html' ) as $html_key ) {
						$html  = (string) ( $row[ $html_key ] ?? '' );
						if ( '' === $html ) {
							continue;
						}
						$fixed = Leadwerk_Volks_Media::rewrite_html_fragment( $html );
						if ( $fixed !== $html ) {
							$value[ $index ][ $html_key ] = $fixed;
							$changed                      = true;
						}
					}
				}
			}
		} elseif ( is_array( $group ) && ! empty( $group['fields'] ) && isset( $value['content'] ) ) {
			$html  = (string) $value['content'];
			$fixed = Leadwerk_Volks_Media::rewrite_html_fragment( $html );
			if ( $fixed !== $html ) {
				$value['content'] = $fixed;
				$changed          = true;
	...
	/**
	 * Parse JSON-LD script blocks from HTML head/body.
	 *
	 * @param string $html Full HTML document.
	 * @return array<int,array<string,mixed>>
	 */
	protected function extract_json_ld_documents( $html ) {
		$html = (string) $html;
		if ( '' === $html || false === stripos( $html, 'application/ld+json' ) ) {
			return array();
		}
		...
		foreach ( $matches[1] as $raw_json ) {
			$decoded = json_decode( trim( html_entity_decode( (string) $raw_json, ENT_QUOTES | ENT_HTML5, 'UTF-8' ) ), true );
			if ( ! is_array( $decoded ) ) {
				continue;
			}
