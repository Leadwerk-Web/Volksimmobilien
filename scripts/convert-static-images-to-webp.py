#!/usr/bin/env python3
"""Convert PNG/JPG/JPEG images to WebP and update references in text files."""
...
TEXT_EXTENSIONS = {".html", ".css", ".js", ".json", ".php", ".md"}
WEBP_QUALITY = 85

# Relative paths (posix) that must stay as PNG/JPG for WP/theme/importer hard-codes.
SKIP_RELATIVE_PATHS = {
    "volksimmobilien_theme/assets/images/logo-placeholder.png",
}

SKIP_FILENAMES = {
    "favicon.png",
    "Volksimmobilien - favicon.png",
}
...
def should_skip_image(root: Path, image_path: Path) -> bool:
    if image_path.name in SKIP_FILENAMES:
        return True
    rel = normalize_path(image_path.relative_to(root))
    if rel in SKIP_RELATIVE_PATHS:
        return True
    if image_path.with_suffix(".webp").exists():
        return True
    return False
...
def collect_images(root: Path):
    ...
    for file_path in root.rglob("*"):
        ...
        if should_skip_image(root, file_path):
            continue
...
def collect_text_files(root: Path):
    ...
    for file_path in root.rglob("*"):
        ...
        # Skip generated reports on re-run (they reference old filenames by design).
        if file_path.name in {"webp-conversion-manifest.json", "webp-conversion-report.json"}:
            continue
...
def run_conversion(root: Path, quality: int, dry_run: bool, delete_originals: bool):
    ...
    print("\nUpdating HTML, CSS, JS, JSON, PHP and MD references...\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Convert PNG/JPG/JPEG images to WebP and update HTML/CSS/JS/JSON/PHP references."
    )
